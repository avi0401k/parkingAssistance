...
uint32_t measure_distance_right();
uint32_t measure_distance_left();
void delay_us(uint32_t us);
#define TRIG_PIN_L GPIO_PIN_3
#define ECHO_PIN_L GPIO_PIN_4

#define TRIG_PIN_R GPIO_PIN_5
#define ECHO_PIN_R GPIO_PIN_6
...
uint32_t measure_distance_right() {
    ...
    HAL_GPIO_WritePin(GPIOA, TRIG_PIN_R, GPIO_PIN_SET);
    delay_us(10);
    HAL_GPIO_WritePin(GPIOA, TRIG_PIN_R, GPIO_PIN_RESET);

    // Wait for echo to go HIGH
    while (HAL_GPIO_ReadPin(GPIOA, ECHO_PIN_R) == GPIO_PIN_RESET);
    ...
    while (HAL_GPIO_ReadPin(GPIOA, ECHO_PIN_R) == GPIO_PIN_SET);
...
uint32_t measure_distance_left() {
    ...
    // Send trigger pulse (10us HIGH)
    HAL_GPIO_WritePin(GPIOA, TRIG_PIN_L, GPIO_PIN_SET);
    delay_us(10);
    HAL_GPIO_WritePin(GPIOA, TRIG_PIN_L, GPIO_PIN_RESET);

    // Wait for echo to go HIGH
    while (HAL_GPIO_ReadPin(GPIOA, ECHO_PIN_L) == GPIO_PIN_RESET);
    ...
    // Wait for echo to go LOW
    while (HAL_GPIO_ReadPin(GPIOA, ECHO_PIN_L) == GPIO_PIN_SET);
